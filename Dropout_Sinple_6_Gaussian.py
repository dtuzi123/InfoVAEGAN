...
def Final_Output(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, last_term, Component_Count):
    # encoding
    mu1, sigma1, mix1 = Create_Encoder_MNIST(x_hat, n_hidden, dim_z, keep_prob, "encoder1",True)
    mu2, sigma2, mix2 = Create_Encoder_MNIST(x_hat, n_hidden, dim_z, keep_prob, "encoder2",True)
    mu3, sigma3, mix3 = Create_Encoder_MNIST(x_hat, n_hidden, dim_z, keep_prob, "encoder3",True)
    ...
    y5 = Create_SubDecoder(z4_samples, n_hidden, dim_img, keep_prob, "decoder5",True)
    y6 = Create_SubDecoder(z4_samples, n_hidden, dim_img, keep_prob, "decoder6",True)
...
def Final_Output_Confusion(z1_samples, z2_samples, z3_samples, z4_samples, dim_img, dim_z, n_hidden, keep_prob):
    ard_init = -10.
    ...
    # decoding
    y1 = Create_SubDecoder(z2_samples, n_hidden, dim_img, keep_prob, "decoder1", True)
    y1 = tf.clip_by_value(y1, 1e-8, 1 - 1e-8)

    y2 = Create_SubDecoder(z3_samples, n_hidden, dim_img, keep_prob, "decoder2", True)
    y2 = tf.clip_by_value(y2, 1e-8, 1 - 1e-8)

    y3 = Create_SubDecoder(z4_samples, n_hidden, dim_img, keep_prob, "decoder3", True)
    y3 = tf.clip_by_value(y3, 1e-8, 1 - 1e-8)

    y4 = Create_SubDecoder(z1_samples, n_hidden, dim_img, keep_prob, "decoder4", True)
    y4 = tf.clip_by_value(y4, 1e-8, 1 - 1e-8)

    # dropout out
    y1 = y1 * dropout_samples[:, 0:1]
    y2 = y2 * dropout_samples[:, 1:2]
    y3 = y3 * dropout_samples[:, 2:3]
    y4 = y4 * dropout_samples[:, 3:4]
...
y, z, loss, neg_marginal_likelihood, KL_divergence,dropout_samples = autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob,
...
with tf.Session() as sess:
    ...
    if isWeight:
        ...
        yy = Final_Output(x_hat, x, dim_img, dim_z, n_hidden, keep_prob,
                                                                 last_term, Component_Count)

        x_fixed = np.reshape(x_fixed, (-1, 28 * 28))

        y1 = sess.run(yy, feed_dict={x_hat:x_fixed,x:x_fixed,keep_prob: 1.0})
    ...
    for epoch in range(n_epochs):
        ...
        if bestScore > loss_likelihood:
            ...
            saver.save(sess, 'models/Dropout_Sinmple_6_Gaussian')
    #saver.save(sess, 'models/Dropout_Sinmple_6_Gaussian')
