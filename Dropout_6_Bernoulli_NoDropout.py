...
y, z, loss, neg_marginal_likelihood, KL_divergence = autoencoder(x_hat, x, dim_img, dim_z,
                                                                                           n_hidden, keep_prob,
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/Dropout_Sinmple_6_Bernoulli_NoDropout')
        '''
    ...
    for epoch in range(n_epochs):
        ...
        if bestScore < loss_likelihood:
            ...
            saver.save(sess, 'models/Dropout_Sinmple_6_Bernoulli_NoDropout')

    # saver.save(sess, 'models/Dropout_Sinmple_6_Bernoulli')

    # save data
    lossArr1 = np.array(lossArr).astype('str')
    f = open("results/lossArr_NoDropout.txt", "w", encoding="utf-8")
