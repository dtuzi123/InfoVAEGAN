...
def Final_Output(z1_samples, z2_samples, z3_samples, z4_samples,z5_samples,z6_samples, mix_samples, dim_img, dim_z, n_hidden, keep_prob):
    ard_init = -10.
...
z1_samples, z2_samples, z3_samples, z4_samples,z5_samples,z6_samples, mix = HiddenOuputs(x_hat, x, dim_img, dim_z, n_hidden, keep_prob,
                                                                   last_term)
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        lossArr.append(loss_divergence)
        sum1 = 0
        for t2 in range(batch_size):
            noCount = 0
            for t1 in range(6):
    ...
    f = open("results/lossArr.txt", "w", encoding="utf-8")
    ...
    f = open("results/Components.txt", "w", encoding="utf-8")
    for i in range(np.shape(lossArr1)[0]):
        f.writelines(lossArr1[i])
        f.writelines('\n')
    f.flush()
    ...
    f = open("results/DropoutRate.txt", "w", encoding="utf-8")
    for i in range(np.shape(lossArr1)[0]):
        f.writelines(lossArr1[i])
        f.writelines('\n')
    f.flush()
