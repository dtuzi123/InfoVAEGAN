...
def InfoVAE(x_hat, z_in, y_in):
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        ...
        q_optim = tf.train.AdamOptimizer(learning_rate * 1, beta1=beta1) \
...
y_in = tf.placeholder(tf.float32, shape=[None, discrete_len + continous_len])
image_dims = [32, 32, 3]
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'F:/Third_Experiment/NewExperiment/InfoVAE/models/InfoVAE_SVNH')
        ...
        batch_c = np.random.uniform(-1, 1, size=(batch_size, continous_len))
        myArr = []
        for k in range(10):
            batch_labels = np.zeros((batch_size, discrete_len))
            batch_labels[:, k] = 1

            batch_codes = np.concatenate((batch_labels, batch_c),
                                         axis=1)
            ...
            ims("results/" + "myhff" + str(0) + ".png", merge2(y_RPR, [8, 8]))

            #for k1 in range(8):
            myArr.append(y_RPR[0])

        myArr = np.array(myArr)
        ims("results/" + "myh" + str(0) + ".png", merge2(myArr, [1, 10]))
        ...
        for epoch in range(n_epochs):
            ...
            saver.save(sess, "models/InfoVAE_SVNH")
