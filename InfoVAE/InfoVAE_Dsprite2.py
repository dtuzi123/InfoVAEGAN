...
def generator(z_in, y_in, batch_size=64, reuse=False):
    with tf.variable_scope("decoder") as scope:
        ...
        '''
        h8 = deconv2d(h6, [batch_size, 64, 64, 1],
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/InfoVAE_Dsprite2')

    else:

        # load dataset
        dataset_zip = np.load('dSpide_dataset\dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz')
        ...
        x_train = np.reshape(x_train, (n_examples, 64, 64, 1))
        ...
        for epoch in range(n_epochs):
            ...
            y_RPR = np.reshape(y_PRR, (-1, 64, 64, 1))
            ims("results/" + "VAE" + str(epoch) + ".jpg", merge2(y_RPR[:64], [8, 8]))

            if epoch > 0:
                x_fixed_image = np.reshape(x_fixed, (-1, 64, 64, 1))
                ims("results/" + "Real" + str(epoch) + ".png", merge2(x_fixed_image[:64], [8, 8]))

            saver.save(sess, "models/InfoVAE_Dsprite2")
