...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, last_term, Component_Count):
    ...
    dropout_samples = dropout_dis.sample(sample_shape=(batch_size, 6))
    dropout_samples = tf.reshape(dropout_samples, (batch_size, 6))
    ...
    loss = -ELBO + kl * p1 + p4 * dHSIC_Value + dropout_regularizer  # + KL_Dropout2(dropout_a)
...
def HiddenOuputs(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, last_term):
    ...
    mu5, sigma5, mix5 = Create_Encoder_MNIST(x_hat, n_hidden, dim_z, keep_prob, "encoder5", True)
    mu6, sigma6, mix6 = Create_Encoder_MNIST(x_hat, n_hidden, dim_z, keep_prob, "encoder6", True)
...
def Final_Output(z1_samples, z2_samples, z3_samples, z4_samples, z5_samples, z6_samples, mix_samples, dim_img, dim_z,
                 n_hidden, keep_prob):
    ard_init = -10.
    ...
    y5 = Create_SubDecoder(z5_samples, n_hidden, dim_img, keep_prob, "decoder5", True)
    y6 = Create_SubDecoder(z6_samples, n_hidden, dim_img, keep_prob, "decoder6", True)
...
y, z, loss, neg_marginal_likelihood, KL_divergence, dropoutA, dropoutSamples = autoencoder(x_hat, x, dim_img, dim_z,
                                                                                           n_hidden, keep_prob,
                                                                                           last_term, Component_Count)
z1_samples, z2_samples, z3_samples, z4_samples, z5_samples, z6_samples, mix = HiddenOuputs(x_hat, x, dim_img, dim_z,
                                                                                           n_hidden, keep_prob,
                                                                                           last_term)
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/Dropout_Sinmple_6_Bernoulli')
        '''
        ...
        reco,dropoutRate,dropoutSamples = sess.run((y,dropoutA,dropoutSamples), feed_dict={x_hat: x_fixed, x: x_fixed, keep_prob: 1.0})
        ...
        yy = Final_Output(z_in1, z_in2, z_in3, z_in4,z_in5,z_in6, mix_in, dim_img, dim_z, n_hidden, keep_prob)

        x_fixed = np.reshape(x_fixed, (-1, 28 * 28))
        z1_samples_, z2_samples_, z3_samples_, z4_samples_,z5_samples_,z6_samples_, mix_ = sess.run(
            (z1_samples, z2_samples, z3_samples, z4_samples,z5_samples,z6_samples, mix),
            feed_dict={x_hat: x_fixed, x: x_fixed, keep_prob: 1.0})
        y1 = sess.run(yy, feed_dict={z_in1: z1_samples_, z_in2: z2_samples_, z_in3: z3_samples_, z_in4: z4_samples_,z_in5:z5_samples_,z_in6:z6_samples_,
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(total_batch):
            ...
            _, tot_loss, loss_likelihood, loss_divergence, dropout_a, dropoutSamples_ = sess.run(
                (train_op, loss, neg_marginal_likelihood, KL_divergence, dropoutA, dropoutSamples),
                feed_dict={x_hat: batch_xs_input, x: batch_xs_target, keep_prob: 1.0})

        listDropoutRate.append(1.0 - dropout_a)
        lossArr.append(tot_loss)
        sum1 = 0
        for t2 in range(batch_size):
            noCount = 0
            for t1 in range(6):
                if dropoutSamples_[t2, t1] < 0.1:
                    noCount = noCount + 1
            yesCount = 6 - noCount
            sum1 = sum1 + yesCount

        sum1 = float(sum1 / batch_size)
        numberComponents.append(sum1)
        print(dropout_a)
        print(sum1)
        ...
        if bestScore < loss_likelihood:
            ...
            saver.save(sess, 'models/Dropout_Sinmple_6_Bernoulli_Fashion')

    # saver.save(sess, 'models/Dropout_Sinmple_6_Bernoulli')

    # save data
    lossArr1 = np.array(lossArr).astype('str')
    f = open("results/lossArr_Fashion.txt", "w", encoding="utf-8")
    ...
    lossArr1 = np.array(numberComponents).astype('str')
    f = open("results/Components_Fashion.txt", "w", encoding="utf-8")
    for i in range(np.shape(lossArr1)[0]):
        f.writelines(lossArr1[i])
        f.writelines('\n')
    f.flush()
    ...
    lossArr1 = np.array(listDropoutRate).astype('str')
    f = open("results/DropoutRate_Fashion.txt", "w", encoding="utf-8")
    for i in range(np.shape(lossArr1)[0]):
        f.writelines(lossArr1[i])
        f.writelines('\n')
    f.flush()
    f.close()
    '''
    xLabels = np.arange(n_epochs)
    import matplotlib.pyplot as plt

    plt.plot(lossArr)
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('The objective value')
    plt.show()

    plt.plot(listDropoutRate)
    plt.xlabel('Epochs')
    plt.ylabel('Dropout rate')
    plt.title('The dropout rate')
    plt.show()

    plt.plot(numberComponents)
