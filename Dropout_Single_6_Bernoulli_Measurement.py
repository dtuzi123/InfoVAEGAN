...
y, z, loss, neg_marginal_likelihood, KL_divergence,dropout_a,dropout_samples = autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob,
...
with tf.Session() as sess:
    ...
    if isWeight:
        ...
        reco = sess.run(y, feed_dict={x_hat: x_fixed, x: x_fixed, keep_prob: 1.0})
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(total_batch):
            ...
            _, tot_loss, loss_likelihood, loss_divergence,dropoutA,dropoutSamples = sess.run(
                (train_op, loss, neg_marginal_likelihood, KL_divergence,dropout_a,dropout_samples),
                feed_dict={x_hat: batch_xs_input, x: batch_xs_target, keep_prob: 1.0})

            listDropoutRate.append(1.0 - dropoutA)
            mySample = dropoutSamples[0]
            tCount = 0
            for t1 in range(6):
                if mySample[t1] == 1.0:
                    tCount = tCount + 1
            numberComponents.append(tCount)
        ...
        listDropoutRate = np.array(listDropoutRate)
        numberComponents = np.array(numberComponents)
        import matplotlib.pyplot as plt
