...
def Final_Output_Confusion(z1_samples, z2_samples, z3_samples, z4_samples,mix_samples, dim_img, dim_z, n_hidden, keep_prob):
    ard_init = -10.
    with tf.variable_scope("", reuse=True):  # root variable scope
        dropout_a = tf.get_variable("dropout")
    ...
    y2 = Create_SubDecoder(z1_samples, n_hidden, dim_img, keep_prob, "decoder2", True)
    # y2 = tf.clip_by_value(y2, 1e-8, 1 - 1e-8)

    y3 = Create_SubDecoder(z2_samples, n_hidden, dim_img, keep_prob, "decoder3", True)
    # y3 = tf.clip_by_value(y3, 1e-8, 1 - 1e-8)

    y4 = Create_SubDecoder(z3_samples, n_hidden, dim_img, keep_prob, "decoder4", True)
    # y4 = tf.clip_by_value(y4, 1e-8, 1 - 1e-8)
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/Dropout_Sinmple_4_Gaussian')

        bc = 0
        sample_arr = []
        gaussian_sample = Samples_of_Gaussian()
        for k1 in range(batch_size):
            gaussian_sample1 = sess.run(gaussian_sample)
            sample_arr.append(gaussian_sample1)
        ...
        # x_fixed = np.reshape(x_fixed,(-1,28,28))
        # ims("results/" + "MNIST__Real" + str(0) + ".png", merge(x_fixed[:64], [8, 8]))
        # ims("results/" + "MNIST__Reco" + str(0) + ".png", merge(reco[:64], [8, 8]))
        ...
        yy = Final_Output_Confusion(z_in1, z_in2, z_in3, z_in4, mix_in, dim_img, dim_z, n_hidden, keep_prob)
        ...
        #z1_samples_ = sample_arr
        #z2_samples_ = sample_arr
        #z3_samples_ = sample_arr
        #z4_samples_ = sample_arr
        ...
        ims("results/" + "b" + str(0) + ".png", merge(yyArr[:100], [5, 20]))

        ims("results/" + "MNIST_Fusion" + str(0) + ".png", merge(yyArr[0:20], [1, 20]))
        ims("results/" + "MNIST_Fusion" + str(1) + ".png", merge(yyArr[20:40], [1, 20]))
        ims("results/" + "MNIST_Fusion" + str(2) + ".png", merge(yyArr[40:60], [1, 20]))
        ims("results/" + "MNIST_Fusion" + str(3) + ".png", merge(yyArr[60:80], [1, 20]))
        ims("results/" + "MNIST_Fusion" + str(4) + ".png", merge(yyArr[80:100], [1, 20]))
    ...
    for epoch in range(n_epochs):
        ...
        if bestScore < recoValue:
            bestScore = recoValue
            saver.save(sess, 'models/Dropout_Sinmple_4_Gaussian')
    saver.save(sess, 'models/Dropout_Sinmple_4_Gaussian')
