...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, last_term, dropout_in):
    ...
    cats_range[:, 0] = 0
    cats_range[:, 1] = 1
    dropout_samples = gumbel_softmax_sample3(dropout_log, dropout_log2, cats_range, [batch_size * 4])
...
def Random_Generation(z):
    # decoding
    y1 = Create_SubDecoder(z, n_hidden, dim_img, keep_prob, "decoder1",reuse=True)
    y2 = Create_SubDecoder(z, n_hidden, dim_img, keep_prob, "decoder2",reuse=True)
    y3 = Create_SubDecoder(z, n_hidden, dim_img, keep_prob, "decoder3",reuse=True)
    y4 = Create_SubDecoder(z, n_hidden, dim_img, keep_prob, "decoder4",reuse=True)

    y1 = Create_FinalDecoder(y1, n_hidden, dim_img, keep_prob, "final",reuse=True)
    y2 = Create_FinalDecoder(y2, n_hidden, dim_img, keep_prob, "final",reuse=True)
    y3 = Create_FinalDecoder(y3, n_hidden, dim_img, keep_prob, "final",reuse=True)
    y4 = Create_FinalDecoder(y4, n_hidden, dim_img, keep_prob, "final",reuse=True)
...
with tf.Session() as sess:
    ...
    if isWeight:
        ...
        p_z2 = distributions.Normal(loc=m2,
                                    scale=v2)
        z_samples_random = p_z2.sample(batch_size)

        z_samples_random1 = sess.run(z_samples_random)
        y1,y2,y3,y4 = Random_Generation(z_in)

        y1,y2,y3,y4 = sess.run((y1,y2,y3,y4),feed_dict={z_in:z_samples_random1, keep_prob: 1.0})
        ...
        myNewImage = np.zeros((40,28,28))
        myNewImage[0:10,:,:] = y1[0:10]
        myNewImage[10:20,:,:] = y2[0:10]
        myNewImage[20:30,:,:] = y3[0:10]
        myNewImage[30:40,:,:] = y4[0:10]

        ims("results/" + "AAA" + str(0) + ".png", merge(myNewImage, [4, 10]))
        ...
        reco, dropoutA, dropoutSamples = sess.run((y, dropoutA, dropoutSamples),
                                                  feed_dict={x_hat: x_fixed, x: x_fixed, keep_prob: 1.0})
        reco = np.reshape(reco, (-1, 28, 28))
        x_fixed = np.reshape(x_fixed, (-1, 28, 28))
        ims("results/" + "MNIST__Real_Bernoulli" + str(0) + ".png", merge(x_fixed[:20], [1, 20]))
        ims("results/" + "MNIST__Reco_Bernoulli" + str(0) + ".png", merge(reco[:20], [1, 20]))
